...
static int32_t _readRawTemperature(struct bmp_t *bmp)
{
    ...
    return 27898;
  #else
    wiringPiI2CWriteReg8(bmp->fd,BMP085_REGISTER_CONTROL, BMP085_REGISTER_READTEMPCMD);
    usleep(5000);
    return (uint16_t)_readReg16BigEndian(bmp->fd,BMP085_REGISTER_TEMPDATA);
  #endif
}
...
bool bmp_create(struct bmp_t **bmp_ret, int32_t sensorID) 
{
  ...
  uint8_t id = wiringPiI2CReadReg8(fd,BMP085_REGISTER_CHIPID);
  if(id != 0x55)
  {
    return false;
  }
